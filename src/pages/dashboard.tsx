...
import { getServiceEngineers } from 'apiSdk/service-engineers';
...
function DashboardPage() {
  ...
  const serviceEngineerStat = useSWR<PaginatedInterface<ServiceEngineerInterface>>(
    () => `/service-engineers`,
    () => getServiceEngineers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: serviceEngineerStat?.data?.totalCount,
      isLoading: serviceEngineerStat.isLoading,
