...
import { getServiceEngineers, deleteServiceEngineerById } from 'apiSdk/service-engineers';
import { ServiceEngineerInterface } from 'interfaces/service-engineer';

type ColumnType = ColumnDef<ServiceEngineerInterface, unknown>;

interface ServiceEngineerListPageProps {
...
export function ServiceEngineerListPage(props: ServiceEngineerListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ServiceEngineerInterface>>(
    () => `/service-engineers?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteServiceEngineerById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ServiceEngineerInterface) => {
    if (hasAccess('service_engineer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/service-engineers/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/service-engineers/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('service_engineer', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/service-engineers/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('service_engineer', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="service_engineer" />
        </Flex>

        {hasAccess('service_engineer', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/service-engineers/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
